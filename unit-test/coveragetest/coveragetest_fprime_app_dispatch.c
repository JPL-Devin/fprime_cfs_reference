...
void Test_FPRIME_APP_TaskPipe(void)
{
    /*
     * Test Case For:
     * void FPRIME_APP_TaskPipe
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        FPRIME_APP_NoopCmd_t Noop;
    } TestMsg;
    ...
    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_APP_MID_ERR_EID, "SAMPLE: invalid command packet,MID = 0x%x");

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(FPRIME_APP_CMD_MID);
    FcnCode   = FPRIME_APP_NOOP_CC;
    MsgSize   = sizeof(TestMsg.Noop);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &MsgSize, sizeof(MsgSize), false);
    FPRIME_APP_TaskPipe(&TestMsg.SBBuf);

    TestMsgId = CFE_SB_ValueToMsgId(FPRIME_APP_SEND_HK_MID);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    FPRIME_APP_TaskPipe(&TestMsg.SBBuf);
...
void Test_FPRIME_APP_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void FPRIME_APP_ProcessGroundCommand
     */
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    union
    {
        CFE_SB_Buffer_t               SBBuf;
        FPRIME_APP_NoopCmd_t          Noop;
        FPRIME_APP_ResetCountersCmd_t Reset;
        FPRIME_APP_ProcessCmd_t       Process;
        FPRIME_APP_DisplayParamCmd_t  DisplayParam;
    } TestMsg;
    UT_CheckEvent_t EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_APP_CMD_LEN_ERR_EID,
    ...
    FPRIME_APP_ProcessGroundCommand(&TestMsg.SBBuf);

    UtAssert_STUB_COUNT(FPRIME_APP_NoopCmd, 1);
    ...
    FcnCode = FPRIME_APP_RESET_COUNTERS_CC;
    ...
    UtAssert_STUB_COUNT(FPRIME_APP_ResetCountersCmd, 1);
    ...
    /* note this will end up calling FPRIME_APP_Process(), and as such it needs to
     * avoid dereferencing a table which does not exist. */
    FcnCode = FPRIME_APP_PROCESS_CC;
    ...
    UtAssert_STUB_COUNT(FPRIME_APP_ProcessCmd, 1);
    ...
    FcnCode = FPRIME_APP_DISPLAY_PARAM_CC;
    ...
    UtAssert_STUB_COUNT(FPRIME_APP_DisplayParamCmd, 1);
    ...
    FcnCode = 1000;
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_APP_CC_ERR_EID, "Invalid ground command code: CC = %d");
    FPRIME_APP_ProcessGroundCommand(&TestMsg.SBBuf);
...
void Test_FPRIME_APP_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool FPRIME_APP_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    ...
    FPRIME_APP_VerifyCmdLength(NULL, size);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &msgid, sizeof(msgid), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    FPRIME_APP_VerifyCmdLength(NULL, size + 1);
...
{
    ADD_TEST(FPRIME_APP_TaskPipe);
    ADD_TEST(FPRIME_APP_ProcessGroundCommand);
    ADD_TEST(FPRIME_APP_VerifyCmdLength);
}
